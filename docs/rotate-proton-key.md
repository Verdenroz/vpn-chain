# Rotating the ProtonVPN WireGuard key (server exit hop)

Rotate whenever the server's Proton WireGuard **private key** may be exposed
...
The new private key travels from your downloaded Proton `.conf` straight to the
...
1. **Mint a new key + revoke the old one.**
   account.protonvpn.com → Downloads → WireGuard → create a **new** configuration
...
2. **Apply it to the server:**
   ```
   server/rotate-proton-key.sh ~/Downloads/proton-new.conf root@<vps>
   ```
   The script parses `PrivateKey` / `Address` / `PublicKey` / `Endpoint` from the
   `.conf`, patches the `proton` outbound in `/usr/local/etc/xray/config.json`
...
3. **Verify the exit IP changed** (with the relay up locally):
   ...
   ```
   It should report the new Proton exit, and `ip-api.com` should still show
   `Proton AG`.

## Notes

- The phone uses a **separate** Proton WG config (see `docs/android.md`); rotate
