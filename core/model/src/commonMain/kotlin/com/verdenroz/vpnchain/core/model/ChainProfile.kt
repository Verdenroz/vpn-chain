...
/**
 * Dedicated Proton WireGuard config for the TUN-mode entry hop; sing-box dials
 * it directly (no separate Proton app needed). Use a device-specific config so
 * it stays independently revocable. Optional — omitting it makes the TUN chain
 * relay-only, exposing the device's real IP to the VPS.
 */
@Serializable
data class ProtonWireGuardEntry(
...
/**
 * Identifiers for the VLESS+REALITY relay and the optional Proton entry hop.
...
data class ChainProfile(
    ...
    val protonEntry: ProtonWireGuardEntry? = null,
) {
    companion object {
